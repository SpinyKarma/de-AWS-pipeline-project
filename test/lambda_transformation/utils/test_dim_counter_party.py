...
def test_that_the_function_return_a_dict_with_desired_keys():
    result=util.counter_party_address_to_dim_counterparty(test_counterparty,test_address)
    assert isinstance(result, dict)

def test_timestamp_carried_over():
    output = util.counter_party_address_to_dim_counterparty(test_counterparty,test_address)
...
def test_address_and_counterparty_join_into_dim_counterparty_dict_when_columns_missing_from_address_csv():
    ...
    with pytest.raises(KeyError):
        util.counter_party_address_to_dim_counterparty(test_counterparty,address_dict_missing_column)
